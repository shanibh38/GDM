...
function createB() {
    ...
    let db = new sqlite3.Database('../GDMI.db', sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
        if (err) {
            fs.appendFileSync('../logB.txt', "Error Connecting to GDMI DB - " + err.message + "\n");
            //  console.error(err.message);
        }
        else {
            fs.appendFileSync('../logB.txt', "Connecting to GDMI DB\n");
        }
    });
    ...
    db.close((err) => {
        if (err) {
            fs.appendFileSync('../logB.txt', "Error GDMI connection closed - " + err.message + "\n");
            //console.error(err.message);
        }
        else {
            fs.appendFileSync('../logB.txt', "GDMI connection closed\n");
...
function insertUsersB(userName, age, gender, education) {
    ...
    let sql = `INSERT INTO Users (userName ,age , gender ,education)
                VALUES  (?,?,?,?)`;
    db.serialize(function () {
...
function updateBonusB(bonus, userName){
    ...
    let sql = `UPDATE Users SET bonus = ? WHERE userName = ?`;
    db.serialize(function () {
