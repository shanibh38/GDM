...
function createA() {
    ...
    let db = new sqlite3.Database('../GDMI.db', sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
        if (err) {
            fs.appendFileSync('../logA.txt', "Error Connecting to GDMI DB - " + err.message + "\n");
            //  console.error(err.message);
        }
        else {
            fs.appendFileSync('../logA.txt', "Connecting to GDMI DB\n");
        }
    });
    ...
    db.close((err) => {
        if (err) {
            fs.appendFileSync('../logA.txt', "Error GDMI connection closed - " + err.message + "\n");
            //console.error(err.message);
        }
        else {
            fs.appendFileSync('../logA.txt', "GDMI connection closed\n");
